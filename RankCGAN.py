...
class RankCGAN(object):
    def __init__(self, args, SUPERVISED=True):
        ...
        self.R = ranker(input_dim=data.shape[1], output_dim=1, input_size=self.input_size)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, x_ in enumerate(self.data_loader):
                ...
                x1_, x2_, r_ = self.pair_loader.__iter__().__next__()
                ...
                if self.gpu_mode:
                    ...
                    x1_, x2_, r_ = x1_.cuda(), x2_.cuda(), r_.float().cuda()
                ...
                R_ = self.R(x_p)
                diff = R_[:self.batch_size//2] - R_[self.batch_size//2:]
                sig = nn.Sigmoid().cuda()
                prob = sig(diff)
